...
This is an Expo + TypeScript + React Native productivity application for BOTH Android and IOS. The codebase follows a strict separation of concerns between mobile UI, API routes, and backend services.
...
npx expo start -c          # Clear cache and launch
...
| `features/`            | Domain-specific React components and logic |
| `services/`            | Backend business logic and integrations    |
| `prisma/schema.prisma` | Database schema definition                 |
| `feature_list.json`    | Feature tracking with pass/fail status     |

## Architecture Rules

- Frontend never accesses Prisma directly. All DB calls go through API routes.
- Backend logic belongs in `services/`, not directly in API routes.
- Use Zustand for global client state and React Query for server state.

## How to Add a Feature

1. Define the database schema in `prisma/schema.prisma` if needed.
2. Create or update backend service in `services/`.
3. Create API route in `app/api/`.
4. Build the UI in `features/<feature-name>/`.
5. Wire data using React Query in the component.
